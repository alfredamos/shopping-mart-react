...
export function DeleteCartItemPage() {
  const [cartItemState, cartItemDispatch] = useReducer<Reducer<CartItemState, CartItemAction>>(
    cartItemReducer,
    new CartItemState()
  );
  ...
  useEffect(() => {
    cartItemService
      ...
      .then((data) => {
        ...
        cartItemDispatch(new CartItemAction(cartItemActions.CART_ITEM_SUCCESS_CART_ITEM, data));
      })
      .catch((error) => {
        cartItemDispatch(new CartItemAction(cartItemActions.CART_ITEM_FAILURE, error));
      });
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [id]);
